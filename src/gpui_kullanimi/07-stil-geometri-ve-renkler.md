...
- `ScaledPixels(f32)` — `Pixels * window.scale_factor()`. Render aracına iletilen değerdir.
...
- Biçim metni (`format string`) her render'da çalışıyorsa `format!` sonucu da her ekran karesinde bellek ayırması üretir; sonucu önbelleğe almak için entity verisinde tutmak gerekir.
