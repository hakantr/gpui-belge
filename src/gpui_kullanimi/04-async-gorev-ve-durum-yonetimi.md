...
```rust
baglam.spawn(async move |baglam| {
    baglam.update(|baglam| {
...
```rust
baglam.spawn(async move |gorunum, baglam| {
    baglam.background_executor().timer(Duration::from_millis(200)).await;
    gorunum.update(baglam, |gorunum, baglam| {
        gorunum.hazir_mi = true;
        baglam.notify();
    })?;
    Ok::<(), anyhow::Error>(())
})
.detach_and_log_err(baglam);
```

**Pencereye bağlı görev.** Pencere bağlamını da async tarafa taşır; `Window` metotları `cx.update_in` ile birlikte kullanılabilir:

```rust
baglam.spawn_in(pencere, async move |gorunum, baglam| {
    gorunum.update_in(baglam, |gorunum, pencere, baglam| {
...
```rust
let gorev = baglam.background_spawn(async move {
    pahali_is().await
});
...
```rust
let yurutucu = baglam.background_executor().clone();
let gorev = baglam.background_spawn(async move {
...
```rust
baglam.spawn_with_priority(Priority::High, async move |baglam| {
    baglam.update(|baglam| {
...
```rust
fn onbellekten_veya_async(onbellekteki: Option<Veri>, baglam: &App) -> Task<anyhow::Result<Veri>> {
    if let Some(veri) = onbellekteki {
        Task::ready(Ok(veri))
    } else {
        baglam.background_spawn(async move { veri_yukle().await })
    }
}
```
...
baglam.spawn_in(pencere, async move |gorunum, baglam| {
    ...
    gorunum.update_in(baglam, |gorunum, pencere, baglam| {
        gorunum.uygula(veri, pencere, baglam);
    })?;
    Ok::<(), anyhow::Error>(())
})
.detach_and_log_err(baglam);
...
baglam.set_global(GenelDurum(durum));

baglam.update_global::<GenelDurum, _>(|genel_durum, _baglam| {
    genel_durum.0.degisti_mi = true;
});

let deger = baglam.read_global::<GenelDurum, _>(|genel_durum, _| genel_durum.0.clone());
```
...
```rust
abonelikler.push(baglam.observe(&diger, |gorunum, diger, baglam| {
    gorunum.kopya = diger.read(baglam).deger;
    baglam.notify();
}));
```
...
baglam.emit(Kaydedildi);

abonelikler.push(baglam.subscribe(&belge, |gorunum, belge, _: &Kaydedildi, baglam| {
...
// new(): self._abonelikler.push(baglam.subscribe(...));

// 2. Detach (geri çağrı view ömrü boyunca yaşar)
baglam.subscribe(&varlik, |...| { ... }).detach();

// 3. Geçici kapsam (elden çıkınca abonelik kalkar)
let _abonelik = baglam.observe(&varlik, |...| { ... });
...
```rust
baglam.observe_new(|calisma_alani: &mut Workspace, pencere, baglam| {
    if let Some(pencere) = pencere {
        calisma_alani.pencere_kancalarini_kur(pencere, baglam);
    }
}).detach();
```
...
```rust
self._aktif_bolme_gozlemi = baglam.observe_in(aktif_bolme, pencere, |gorunum, bolme, pencere, baglam| {
    gorunum.bolmeden_esitle(&bolme, pencere, baglam);
});

self._abonelik = baglam.subscribe_in(&modal, pencere, |gorunum, modal, olay, pencere, baglam| {
    gorunum.modal_olayini_isle(modal, olay, pencere, baglam);
});
```
...
```rust
let bolme_rezervasyonu: Reservation<Pane> = baglam.reserve_entity();
let bolme_id = bolme_rezervasyonu.entity_id();

let calisma_alani = baglam.new(|baglam| {
    Workspace::with_pane_id(bolme_id, baglam)
});

let bolme = baglam.insert_entity(bolme_rezervasyonu, |baglam| {
    Pane::new(calisma_alani.downgrade(), baglam)
});
```
...
impl Onizleme {
    fn new(baglam: &mut Context<Self>) -> Self {
        let onbellek = RetainAllImageCache::new(baglam);
        let abonelik = baglam.observe_release(&onbellek, |gorunum, _onbellek, baglam| {
...
```rust
let sizinti_anlik_gorunumu = baglam.leak_detector_snapshot();
// Test gövdesi.
baglam.assert_no_new_leaks(&sizinti_anlik_gorunumu);
```
