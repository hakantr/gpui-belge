...
| Mermaid tema tüketimi | `markdown/src/mermaid.rs` | Mermaid renderer teması aktif `ThemeColors`, `AccentColors`, `PlayerColors`, `Appearance` ve `ThemeSettings::ui_font` üstünden üretilir. `Hsla` renkleri renderer'a alpha'sız `#rrggbb` olarak verilir; tema/settings değişiminde cache invalidate edilmelidir. `accent0..accentN` class'ları player slot'larından gelir, pie renkleri ise `AccentColors::color_for_index` kullanır |
