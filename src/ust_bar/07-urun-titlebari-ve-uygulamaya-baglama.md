...
Update bildiriminin görsel kabuğu `crates/ui/src/components/collab/update_button.rs` içindeki `UpdateButton` tipidir. `UpdateVersion::Render`, auto-update durumuna göre beş constructor'dan birini seçer: `checking`, `downloading`, `installing`, `updated`, `errored`. Bunlardan ilk üçü (`Checking for Zed Updates…`, `Downloading Zed Update…`, `Installing Zed Update…`) tıklamaya kapalıdır; constructor'larında `disabled(true)` çağrılır ve render edilen düğme `disabled` bayrağına geçer. Bu üç durumda butonun sınır rengi `colors().border` üzerinden gelir; `updated` ve `errored` durumlarında ise sınır `colors().text.opacity(0.15)` ile çizilir. İkonografi de aynı şekilde durum-koşullu seçilir: `Checking` ve `Installing` `IconName::LoadCircle` ile iki turluk bir dönüş animasyonu uygular, `Downloading` durağan `Download` ikonu kullanır, `errored` ise uyarı rengiyle `Warning` ikonunu gösterir ve dismiss tutamağı ekler. Errored etiketi `"Failed to Update"` biçimindedir; daha önceki `"Failed to update Zed"` metni bırakılmaz. Port hedefinde aynı bileşen kurulurken bu beş durumun her biri ayrı bir constructor olarak ifade edilirse, ürün titlebar'ı auto-update state machine'inin dilini doğrudan yansıtır ve durum kayışı sırasında butonun yanlışlıkla tıklanabilir kalması engellenir.

Skills duyurusu, ürün titlebar'ındaki güncel banner örneğidir. `TitleBar::new(...)` artık `banner: None` ile başlamaz; `OnboardingBanner::new(...)` sonucu `Some(...)` olarak kurulur ve görünürlüğü `.visible_when(|cx| cx.has_flag::<SkillsFeatureFlag>())` predicate'iyle belirlenir. Banner etiketi migration durumundan bağımsız olarak sabittir: prefix `"Introducing:"`, ana metin `"Skills"` olur. Kullanıcının Rules taşıyacak içeriği olup olmadığı banner metninde değil, tıklamayla açılan migration bilgi modalında anlatılır. Port hedefinde eski Claude Agent veya ACP duyuru kalıntıları için uyumluluk yolu bırakılmaz; bu alan tek bir güncel duyuru modeli olarak ele alınır.
...
- `system_window_tabs.rs` içindeki action ve settings kullanımları ürünün kendi action ve settings tipleriyle değiştirilir. Bu maliyetli görünüyorsa native tab desteği proje kararına göre kapalı başlatılır ve ihtiyaç doğduğunda etkinleştirilir. Buradaki bağımlılıklar diğer parçalara göre daha geniştir.
